...
const PatientDetails: React.FC<PatientDetailsProps> = ({ patient, onDelete }) => {
    ...
    const [oxygenLevelData, setOxygenLevelData] = useState<{ time: string; SpO2: number }[]>([])
    ...
    useEffect(() => {
        ...
        ws.onmessage = (event) => {
            ...
            if (data.device_code === patient.device_id) {
                ...
                setHeartRateData((prev) => [...prev.slice(-23), { time, HeartRate: data.heartRate }])
                setOxygenLevelData((prev) => [...prev.slice(-23), { time, SpO2: data.oxygen }])

                const avgHR = [...heartRateData, { time, HeartRate: data.heartRate }].reduce((sum, val) => sum + val.HeartRate, 0) / (heartRateData.length + 1)
